class Pawn:
    ...
    def turn(board, start_cage, target_cage, check_for_attack=False):
        ...
        if abs(start_cage[1] - target_cage[1]) == abs(start_cage[0] - target_cage[0]) and \
            ...
            return "ok"
        print((type(board[start_cage[0], target_cage[1] - operand]) is Pawn or
                                        type(board[start_cage[0], target_cage[1] + operand]) is Pawn))
        if abs(start_cage[1] - target_cage[1]) == abs(start_cage[0] - target_cage[0]) and \
                ((board[start_cage].color == 'w' and target_cage[0] > start_cage[0]) or
                 (board[start_cage].color == 'b' and target_cage[0] < start_cage[0])) and (type(
            (board.prev_pos[target_cage[0], target_cage[1] - operand]) is Pawn or type(
                board.prev_pos[target_cage[0], target_cage[1] + operand]) is Pawn)) \
                and (board.empty((target_cage[0], target_cage[1] - operand)) or board.empty((target_cage[0],
                                                                                             target_cage[1] + operand))
        ) and print(")))") is None and (type(board[start_cage[0], target_cage[1] - operand]) is Pawn or
                                        type(board[start_cage[0], target_cage[1] + operand]) is Pawn):
            if not check_for_attack:
                if board[start_cage].color == 'w':
                    board[target_cage[0] + operand, target_cage[1]] = board.clear_cage
                else:
                    board[target_cage[0] - operand, target_cage[1]] = board.clear_cage
            return "ok"
